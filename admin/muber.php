    ...
    if ($do == "manage") {   //manger redirect
                ...
                <table class="main-table table table-bordered text-center">
                    ...
                    foreach ($rows as $row) {
                        ...
                        <tr>
                            ...
                            <td><img src="layout/images/uploads/<?= $row['Avatar'] ?>" width="50px" alt=""></td>
                            <td>
                                <a href="?do=edite&id=<?= $row['UserID'] ?>&gid=0" class="btn btn-success"><i
                                            class="fa fa-edit"></i>Edit</a>
                                <a href="?do=delete&id=<?= $row['UserID'] ?>" class="btn btn-danger confirm"><i
                                            class="fa fa-trash"></i>Delete</a>
                                <?php
                                if ($row['RegStatus'] == 0) {
                                    ?>
                                <a href="?do=activet&id=<?= $row['UserID'] ?>&gid=0" class="btn btn-info"><i
                                                class="fa fa-edit"></i>Active</a><?php
                                }
                                ?>
                            </td>
                        </tr>
        ...
        if ($count > 0) {
            ...
            <div class="container edit">
                <form method="POST" action="muber.php?do=update&gid=<?= $gid ?>" enctype="multipart/form-data">
                    ...
                    <div class="form-group row">
                        <label for="Avatar" class="col-sm-1 col-form-label">Avatar</label>
                        <div class="col-sm-10 my-form-group">
    ...
    elseif ($do == "update") { //update page
        ...
        <div class="container">
            ...
            if ($_SERVER['REQUEST_METHOD'] == "POST") {
                ...
                if (isset($_POST['edite']) && isset($_FILES['avatar'])) {
                ...
                if (empty($formErrors)) {
                    ...
                    if ($resCheck == 0) {
                        ...
                        $stmt = $conn->prepare($sql);
                        $stmt->execute(array($username, $password, $email, $fullname, $imageRandName,  $userid, $gid));
                        $res = $stmt->rowCount();
...
//                echo "you can not browes the page diractory";
        ...
        <div class="container">
            ...
            if ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['add'])) {
                ...
                if (isset($_POST['add']) && isset($_FILES['avatar'])) {
                    $file_name = $_FILES['avatar']['name'];
                    $file_type = $_FILES['avatar']['type'];
                    $file_size = $_FILES['avatar']['size'];
                    $file_tem_loc = $_FILES['avatar']['tmp_name'];
...
//                if ($stmtChack->rowCount() != 1) {
                        ...
                        $sql = "INSERT INTO 
                            usres (Username, Password, Email, Fullname, Avatar, RegStatus, RegusterDate)
                            VALUES (:username, :password, :email, :fullname, :avatar, 1, now())";
                        $stmt = $conn->prepare($sql);
                        try {
                            $stmt->execute(array("username" => $username, "password" => $hashPassword,
                                "email" => $email, "fullname" => $fullname, "avatar"=>$imageRandName));
                            $res = $stmt->rowCount();
                            ?>
